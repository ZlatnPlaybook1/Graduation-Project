...
const Quiz = ({ questions }) => {
  ...
  const handleAnswerSelect = (questionId, answer, correctAnswer) => {
    ...
    if (answer === correctAnswer) {
      setAnswerStatus({ status: "correct", selected: answer, correct: correctAnswer });
    } else {
      setAnswerStatus({ status: "wrong", selected: answer, correct: correctAnswer });
    }
    ...
    <main>
      ...
      <form className="show-section">
        <section className="steps">
          <div className="step-number">Question {currentQuestionIndex + 1} / {questions.length}</div>
            ...
            {currentQuestion.options.map((option, index) => (
              <div 
                ...
                <input
                  ...
                  onChange={() => handleAnswerSelect(currentQuestion.id, option, currentQuestion.correct_answer)}
                />
                <label>
                  {option} 
                  {answerStatus?.selected === option && answerStatus.status === "correct" && " ✅"}
                  {answerStatus?.selected === option && answerStatus.status === "wrong" && " ❌"}
                </label>
              </div>
            ))}
          </fieldset>
