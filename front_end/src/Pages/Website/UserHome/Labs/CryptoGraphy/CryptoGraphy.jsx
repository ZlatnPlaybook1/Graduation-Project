        ...
        <div className="container" ref={faqSectionRef}>
          <div className="row">
            ...
            <dt className="wave-labd fadeInUp faq-header-labd">
                ...
                <br></br>
                <p>To recover the plaintext, we must pass the ciphertext along with the proper key via the decryption function, which would give us the original plaintext. This is shown in the illustration below.</p>
                <br></br>
            ...
            <dt className="wave-labd fadeInUp faq-header-labd">
                ...
                <br></br>
                <p>For encryption, we shift to the right by three; for decryption, we shift to the left by three and recover the original plaintext, as illustrated in the image above. However, if someone gives you a ciphertext and tells you that it was encrypted using Caesar Cipher, recovering the original text would be a trivial task as there are only 25 possible keys. The English alphabet is 26 letters, and shifting by 26 will keep the letter unchanged; hence, 25 valid keys for encryption with Caesar Cipher. The figure below shows how decryption will succeed by attempting all the possible keys; in this case, we recovered the original message with <span class="math inline"><em>K</em><em>e</em><em>y</em> = 5</span>. Consequently, by today’s standards, where the cipher is publicly known, Caesar Cipher is considered insecure.</p>
                <br></br>
                ...
                <br></br>
                <p><strong>Symmetric encryption</strong>, also known as <strong>symmetric cryptography</strong>, uses the same key to encrypt and decrypt the data, as shown in the figure below. Keeping the key secret is a must; it is also called <strong>private key cryptography</strong>. Furthermore, communicating the key to the intended parties can be challenging as it requires a secure communication channel. Maintaining the secrecy of the key can be a significant challenge, especially if there are many recipients. The problem becomes more severe in the presence of a powerful adversary; consider the threat of industrial espionage, for instance.</p>
                ...
                <br></br>
                <p>Unlike symmetric encryption, which uses the same key for encryption and decryption, <strong>asymmetric encryption</strong> uses a pair of keys, one to encrypt and the other to decrypt, as shown in the illustration below. To protect confidentiality, asymmetric encryption or <strong>asymmetric cryptography</strong> encrypts the data using the public key; hence, it is also called <strong>public key cryptography</strong>.</p>
                <br></br>
                <img id="img-full" src={exampleImage9} alt="Example" />
                <br></br>
                <p>Examples are RSA , Diffie-Hellman, and Elliptic Curve cryptography (<span >ECC</span>). The two keys involved in the process are referred to as a <b>public key</b> and a <b>private key</b>. Data encrypted with the public key can be decrypted with the private key. Your private key needs to be kept private, hence the name.</p>
                <p>Asymmetric encryption tends to be slower, and many asymmetric encryption ciphers use larger keys than symmetric encryption. For example, <span >RSA</span> uses 2048-bit, 3072-bit, and 4096-bit keys; 2048-bit is the recommended minimum key size. Diffie-Hellman also has a recommended minimum key size of 2048 bits but uses 3072-bit and 4096-bit keys for enhanced security. On the other hand, <span >ECC</span> can achieve equivalent security with shorter keys. For example, with a 256-bit key, <span >ECC</span> provides a level of security comparable to a 3072-bit <span >RSA</span> key.</p>
                <p>Asymmetric encryption is based on a particular group of mathematical problems that are easy to compute in one direction but extremely difficult to reverse. In this context, extremely difficult means practically infeasible. For example, we can rely on a mathematical problem that would take a very long time, for example, millions of years, to solve using today’s technology.</p>
                <p>We will visit various asymmetric encryption ciphers in the next room. For now, the important thing to note is that asymmetric encryption provides you with a public key that you share with everyone and a private key that you keep guarded and secret.</p>
                <br></br>
